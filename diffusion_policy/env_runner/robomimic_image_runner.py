...
class RobomimicImageRunner(BaseImageRunner):
    ...
    def run(self, policy: BaseImagePolicy):
        ...
        for chunk_idx in range(n_chunks):
            ...
            if (
                isinstance(policy, ActionChunkTransformerPolicy)
                and policy.temporal_agg is True
            ):
                all_env_time_actions = torch.zeros(
                    [
                        n_envs,
                        self.max_steps,
                        self.max_steps + policy.num_queries,
                        policy.action_dim,
                    ]
                ).to(device)
            ...
            while not done:
                ...
                if all_env_time_actions is not None:
                    ...
                    original_action = action_dict["action_pred"]
                    all_env_time_actions[
                        :, ts, ts : ts + policy.num_queries, :
                    ] = original_action

                    # average actions for each env
                    all_env_curr_actions = torch.zeros(
                        [n_envs, 1, policy.action_dim]
                    ).to(device)
                    for env_idx, per_env in enumerate(all_env_time_actions):
                        ...
                        exp_weights = (
                            torch.from_numpy(exp_weights).to(device).unsqueeze(dim=1)
                        )
                        raw_action = (act_for_curr_step * exp_weights).sum(
                            dim=0, keepdim=True
                        )
