...
def test():
    ...
    cfg_path = os.path.expanduser('~/diffusion_policy/diffusion_policy/config/task/lift_image.yaml')
    cfg = OmegaConf.load(cfg_path)
    cfg['n_obs_steps'] = 1
    cfg['n_action_steps'] = 1
    cfg['past_action_visible'] = False
    runner_cfg = cfg['env_runner']
    runner_cfg['n_train'] = 5
    runner_cfg['n_test'] = 5
    del runner_cfg['_target_']
    runner = RobomimicImageRunner(
        **runner_cfg, 
        output_dir='/home/clear/diffusion_policy/data/tmp/test')
